...
export default async (req: Request) => {
  ...
  try {
    const { prompt, currentText } = await req.json();
    ...
    if (!apiKey) {
      console.warn("GEMINI_API_KEY non configurata nei segreti di Netlify.");
      throw new Error("GEMINI_API_KEY is not defined. Please configure it in your Netlify Environment Variables.");
    }
...
`;
    ...
    console.error('Gemini Netlify processing error:', error);
    
    // Exact same graceful fallbacks as server.ts to be robust on Netlify!
    const mockResponses = [
      {
